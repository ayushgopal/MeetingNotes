...
class ActionAddActionItem(Action):
    ...
    async def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        owners_ref=["sanika","aayush","ayushman","neeshi","samarth","kanik","amit","ravin","tanmay"]
        owners=tracker.get_latest_entity_values("owner")
        print(list(owners))
        if list(owners)==[]:
            owner=tracker.get_slot('owner')
            matches=get_close_matches(owners_ref,owner)
            print(matches)
        owner_list=[]
        for owner in owners:
