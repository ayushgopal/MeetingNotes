...
class ActionAddTalkingPoint(Action):
    ...
    async def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        for user in users:
            user_full_names.append(user["name"])

        for user_name in user_full_names:
            split=[]
            split=user_name.split()
            user_first_names.append(split[0])
        ...
        for user in users:
            if user["name"].lower()==speaker.lower():
                ...
                dispatcher.utter_message("Speaker is not registered in the database. Using default speaker name instead")
                created_by["name"]="Default Speaker"
                created_by["user_id"]="Default id"
...
class ActionAddActionItem(Action):
    ...
    async def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        owners_ref=user_full_names+user_first_names
        ...
        user_first_names_lower=[name.lower() for name in user_first_names]
        ...
        for owner in owner_list:
            ...
            elif owner.lower() in user_first_names_lower:
                for name in user_first_names:
                    if name.lower()==owner.lower():
                        index=user_first_names.index(name)
                        user=users[index]
                        info["name"]=user["name"]
                        info["role"]="owner"
                        info["color"]=user["color"]
        ...
        if(upload.status_code == 200):
            ...
            print("api/v1/notes/group/note/json/data/update/")
            print("Upload Unsuccessful", upload.status_code)
